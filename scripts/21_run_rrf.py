...
from poc.rrf import run_wands_rrf_benchmark
...
def main() -> None:
    ...
    with OpenSearchClient(url, timeout=300) as client:
        ...
        summary = run_wands_rrf_benchmark(
            client,
            root=ROOT,
    ...
    print(
        f"selected {summary['model']} with RRF rank constant "
        f"{cast(int, summary['selected_rank_constant'])} on development; "
        f"held-out NDCG@10={test_metrics['ndcg@10']:.6f}, "
        f"delta_vs_tuned_bm25="
        f"{cast(float, summary['rrf_minus_tuned_bm25_ndcg@10']):+.6f}, "
