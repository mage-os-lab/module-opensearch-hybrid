...
from poc.ann_recall import verify_wands_ann_recall
...
def main() -> None:
    ...
    with OpenSearchClient(url, timeout=300) as client:
        result = verify_wands_ann_recall(
            client,
            root=ROOT,
            index=index.name,
            model=model,
    ...
    print(f"verified ANN recall gate: {result['quality_evidence_eligible_for_decision']}")
