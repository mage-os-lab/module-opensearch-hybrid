...
from poc.end_to_end_latency import load_latency_protocol, verify_latency_result
...
def main() -> None:
    ...
    path = (
        ROOT
        / f"results/wands/latency/routed-sparse-{platform.machine().lower()}.json"
    )
    ...
    print(
        "verified routed sparse latency: "
        f"decision_eligible={result['latency_decision_eligible']}"
    )
