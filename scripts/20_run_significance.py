...
from poc.significance import build_wands_hybrid_significance
...
def main() -> None:
    ...
    with OpenSearchClient(url, timeout=300) as client:
        result = build_wands_hybrid_significance(
            client,
            root=ROOT,
            index=index,
            models=models,
    ...
    for name, comparison in comparisons.items():
        print(
            f"{name} vs tuned_bm25: "
            f"delta={float(comparison['paired_mean_delta']):+.6f}, "
    ...
    for name, comparison in challenge_comparisons.items():
        print(
            f"{name} vs competent_integrator_bm25: "
            f"delta={float(comparison['paired_mean_delta']):+.6f}, "
            f"Holm p={float(comparison['holm_adjusted_p_value']):.9f}, "
            f"interpretation={comparison['interpretation']}"
        )
