...
from poc.significance import verify_wands_sparse_significance
...
def main() -> None:
    ...
    with OpenSearchClient(url, timeout=300) as client:
        result = verify_wands_sparse_significance(
            client,
            root=ROOT,
            index=index,
            models=models,
    ...
    print(f"verified sparse significance for {len(comparisons)} candidate arms")
